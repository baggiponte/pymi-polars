...
@build:
  npx slidev build --base=/pymi-polars
...
[private]
  ...
  #! /usr/bin/env zsh
  if ! gh repo list baggiponte | grep --quiet "/\bpymi-polars\b"; then
    while true; do
      ...
      case $REPLY in
        [Yy])
          ...
          case $PRIVATE in
            [Yy])
              gh repo create baggiponte/pymi-polars --private --source=.
              ;;
            [Nn])
              gh repo create baggiponte/pymi-polars --public --source=.
              ;;
            *)
    ...
    while true; do
      ...
      case $REPLY in
        [Yy])
          git remote add origin git@github.com:baggiponte/pymi-polars
          break
          ;;
        [Nn])
          break
